...
A sequence data type is the data type that supports the membership operator _in_, the size function _len()_, slices _[]_ and iterable.
...
- In Python a list is an **_ordered sequence_** of zero or more object references.
...
> If a binary operator is used with a set and frozenset then the result will be <br/> > `f&s => returns frozenset <br/>` <br/> > `s&f => returns set` <br/> > `== or != => returns True/False`
...
Note: [dict's retaining insertion is guaranteed from python3.7](https://stackoverflow.com/questions/39980323/are-dictionaries-ordered-in-python-3-6), So, no need to specifically use OrderedDict
